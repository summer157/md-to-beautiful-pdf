...
```bash
mdpdf input.md --theme purple-typora --output report.pdf
```
...
- `minimal`：克制、干净、适合打印。
- `purple-typora`：紫色强调色，从 Typora purple CSS 主题迁移而来，适合技术说明和分享文档。

两个主题都是普通 `.typ` 文件，位于 `themes/` 目录，可以直接编辑或以此为基础创建自己的主题。
...
这样不会覆盖内置的 `purple-typora` 主题。

指定主题名称：

```bash
mdpdf import-theme purple.css --name my-purple
```

使用导入后的主题：

```bash
mdpdf input.md --theme my-purple
```
...
mdpdf examples/technical-report.md --theme purple-typora --title "架构设计方案"
```

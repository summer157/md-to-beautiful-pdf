...
function cssColorToTypst(raw: string): string {
  const color = raw.trim()
  if (!color || color === 'transparent') return 'rgb("#00000000")'
  ...
  if (named[color]) return `rgb("${named[color]}")`
...
export interface ThemeTokens {
  ...
  // palette
  accent: string              // primary accent (title color)
  textColor: string           // body text
  lightText: string           // secondary text
  linkColor: string           // hyperlink
  ...
  // structural
  borderColor: string         // table/hr/general border
  blockquoteBorder: string    // blockquote left border color

  // fonts (first-choice name, full list)
  bodyFonts: string[]
  headingFonts: string[]
  codeFonts: string[]

  // heading decoration flags
  h1Centered: boolean
  h1BottomRule: boolean       // has :after border-bottom
  h1HeadingWeight: string     // 'bold' | 'normal'
...
export function extractTokens(cssPath: string): ThemeTokens {
  ...
  const getVarAny = (names: string[], fallback: string): string => {
    for (const name of names) {
      const v = vars.get(name)
      if (v) return resolveVars(v, vars)
    }
    return fallback
  }
  ...
  const accent = getVarAny(
    ['--title-color', '--accent-color', '--primary-color', '--theme-color',
  ...
  const textColor = getVarAny(
    ['--text-color', '--body-color', '--fg-color', '--foreground', '--content-color'],
    '#333333',
  )
  const lightText = getVarAny(
    ['--light-text-color', '--muted-text-color', '--meta-text-color',
     '--secondary-text', '--lighter-text-color', '--subtitle-color'],
    '#666666',
  )
  const linkColor = getVarAny(
    ['--link-color', '--a-color', '--hyperlink-color'],
    '#1a73e8',
  )

  // inline code
  const inlineCodeColor = getVarAny(
    ['--code-color', '--inline-code-color', '--code-fg', '--code-text-color'],
    accent,
  )
  const inlineCodeBg = getVarAny(
    ['--inline-code-bg', '--inline-code-bg-color', '--code-bg', '--code-background'],
    '#f4f2f9',
  )
  ...
  // borders
  const borderColor = getVarAny(
    ['--border', '--border-color', '--line-color', '--divider-color', '--separator-color'],
    '#e7e7e7',
  )
  const blockquoteBorder = getVarAny(
    ['--border-quote', '--blockquote-border', '--quote-border',
     '--blockquote-accent', '--blockquote-color'],
    accent,
  )

  // ── Step 3: fonts ─────────────────────────────────────────────────────────
  const bodyFonts = parseFontFamilies(
    getVarAny(['--base-font', '--body-font', '--font-family', '--text-font'], 'PingFang SC, Arial'),
  )
  const headingFonts = parseFontFamilies(
    getVarAny(['--title-font', '--heading-font', '--h-font', '--base-font', '--font-family'],
              bodyFonts.join(', ')),
  )
  const codeFontsRaw = parseFontFamilies(
    getVarAny(['--monospace', '--code-font', '--mono-font', '--font-monospace'], 'Menlo, Courier New'),
  )
...
function generateH1(t: ThemeTokens): string {
...
function generateH2(t: ThemeTokens): string {
  const accent = cssColorToTypst(t.accent)
  const weight = `"${t.h1HeadingWeight}"`
...
export function generateTypstTheme(tokens: ThemeTokens): string {
  const c = (raw: string) => cssColorToTypst(raw)
  const fonts = (list: string[]) => typstFontList(list)

...
export async function convert(opts) {
    ...
    if (!hasPandoc) {
        throw new Error('pandoc is not installed or not in PATH.\n' +
            '  Install: https://pandoc.org/installing.html\n' +
            '  macOS:   brew install pandoc');
    }
    if (!hasTypst) {
        throw new Error('typst is not installed or not in PATH.\n' +
    ...
    try {
        ...
        if (existsSync(tempTypPath))
            unlinkSync(tempTypPath);
    }
}
